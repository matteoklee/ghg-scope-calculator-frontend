  ...
  data() {
    return {
      resultItems: [
        ...
        {
          title: 'Indirekte Emissionen',
          subTitle: 'Scope 2 + Scope 3',
          emission: 49.62,
          percentage: 28.2
        }
      ]
    ...
    <div class="m-12">
      ...
      <div class="mt-8">
        <div class="grid xl:grid-cols-3 grid-cols-1 gap-8">
          <div v-for="(item, index) in resultItems" :key="index" class="border shadow rounded-lg p-6">
            <p class="text-2xl font-bold mb-2">{{item.title}}</p>
            <p class="text-gray-700 mb-2">{{item.subTitle}}</p>
            <p class="text-xl text-gray-500 mb-2"><span class="text-4xl font-bold text-black mr-2">{{item.emission}}</span>tCO2e</p>
